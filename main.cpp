...
// Function to generate and validate Serial Ids
...
void writingDataInFile(string serialId, string bookName, string author, int shelfNo, string serialIntoBinary);
...
main()
{
    ...
    while (true)
    {
        ...
        if (choices == "1")
        {
            ...
            writingDataInFile(serialId, bookName, author, shelfNo, serialToBianry);
            ...
            if (verifyInputFormat(serialId))
            {
                ...
                if (record != "")
                {
                    if (generateCheckDigit(serialNo, shelfNo) == checkDigit)
                    {
                        ...
                        cout << "ShelfNo: " << readingDataFromField(record, 4) << endl;
                        cout << "Barcode: " << endl;
                        printBarcode(readingDataFromField(record, 5));
                    }
                    else
                    {
...
void writingDataInFile(string serialId, string bookName, string author, int shelfNo, string serialToBinary)
{
    fstream file;
    file.open("data.txt", ios::app);
    file << serialId << "," << bookName << "," << author << "," << to_string(shelfNo) << "," << serialToBinary << ",";
    file << endl;
    file.close();
}
...
void printAllSerial()
{
    ...
    while (getline(file, record))
    {
        recordExist = true;
        cout << "------------------------------------------" << endl;
        cout << "Book:      " << readingDataFromField(record, 1) << endl;
        cout << "Author:    " << readingDataFromField(record, 2) << endl;
        cout << "Serial ID: " << readingDataFromField(record, 3) << endl;
        cout << "Shelf No: " << readingDataFromField(record, 4) << endl;
        cout << "Barcode: " << endl;
        printBarcode(readingDataFromField(record, 5));
        cout << "------------------------------------------" << endl;
    }
